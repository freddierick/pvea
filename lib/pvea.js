...
global.doTicketNeedsRefresh = true
// pvea module
class pvea {
  ...
  async run(callback) {
    ...
    await axios(options)
      .then(res => {
        global.ticket = `PVEAuthCookie=${  res.data.data.ticket}`
        global.csrfToken = res.data.data.CSRFPreventionToken
    ...
    global.doTicketNeedsRefresh = false
    setTimeout(function(){ global.doTicketNeedsRefresh = true }, 3600000)
    }
    if (callback) {
      await callback()
    }
